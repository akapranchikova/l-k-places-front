...
export class AddMarkerModalComponent implements OnInit, AfterViewInit {
  ...
  save() {
    ...
    formDataPoster.append('mapMarker', JSON.stringify(data));
    if (this.image) {
      formDataPoster.append('image', this.image);
    }

    this.httpService.post('/map-markers', formDataPoster, true, 'multipart/form-data').subscribe(res => this.dialogRef.close(true));
  }
